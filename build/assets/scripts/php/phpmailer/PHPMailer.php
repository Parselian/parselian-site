...
try {
$this->error_count = 0; $this->mailHeader = '';
...
$prefix = 'ssl://';
$tls = false; $secure = static::ENCRYPTION_SMTPS;
...
if($this->SMTPAuth && !$this->smtp->authenticate($this->Username,
$this->Password,
$this->AuthType,
$this->oauth
)
...
$matches = [];
if(preg_match('/^\$PHPMAILER_LANG\[\'([a-z\d_]+)\'\]\s*=\s*(["\'])(.+)*?\2;/',
$line,
$matches
) &&
...
$result .= $this->headerLine(trim($header[0]),
...
$mime[] = sprintf('Content-Disposition: %s%s',
$disposition,
static::$LE . static::$LE
);
}
...
public function addEmbeddedImage($path,
$cid,
$name = '',
$encoding = self::ENCODING_BASE64,
$type = '',
...
public function addStringEmbeddedImage($string,
$cid,
$name = '',
$encoding = self::ENCODING_BASE64,
$type = '',
...
{
$this->RecipientsQueue = array_filter($this->RecipientsQueue,
...
$cid = substr(hash('sha256', $data), 0, 32) . '@phpmailer.0'; if(!$this->cidExists($cid)) {
$this->addStringEmbeddedImage($data,
$cid,
'embed' . $imgindex,
...
$message = str_replace($images[0][$imgindex],
...
if($this->addEmbeddedImage($basedir . $directory . $filename,
$cid,
$filename,
static::ENCODING_BASE64,
...
{
$DKIMsignatureType = 'rsa-sha256'; $DKIMcanonicalization = 'relaxed/simple'; $DKIMquery = 'dns/txt'; $DKIMtime = time();
$autoSignHeaders = [
'from',
'to',
'cc',
...
$copiedHeaderFields .= substr(chunk_split($copiedHeader, self::STD_LINE_LENGTH - 3, static::$LE . self::FWS),
0,
